    ...
    return (
      <div className="min-h-screen bg-[#0A0A0A] text-white flex items-center justify-center px-4">
        <div className="text-center max-w-md">
          <img src={LOGO_URL} alt="After Hours" className="h-16 w-auto mx-auto mb-6 rounded-2xl" />
    ...
    <div className="min-h-screen bg-[#0A0A0A] text-white relative">
      ...
      <div className="container mx-auto px-4 sm:px-6 pt-16 pb-12 sm:pb-20 max-w-2xl">
        ...
        <div className="space-y-6">
          ...
          <div>
            <label className="block text-sm font-medium mb-2">Full Name <span className="text-[#C9A84C]">*</span></label>
            <input type="text" name="full_name" value={formData.full_name} onChange={handleChange} placeholder="John Doe" className="w-full px-4 py-3 rounded-xl bg-[#141414] border border-[#2A2A2A] text-white placeholder-[#666] focus:outline-none focus:border-[#C9A84C]/50 focus:shadow-[0_0_10px_rgba(201,168,76,0.1)] transition-all" />
          </div>

          <div>
            <label className="block text-sm font-medium mb-2">Email <span className="text-[#C9A84C]">*</span></label>
            <input type="email" name="email" value={formData.email} onChange={handleChange} placeholder="you@example.com" className="w-full px-4 py-3 rounded-xl bg-[#141414] border border-[#2A2A2A] text-white placeholder-[#666] focus:outline-none focus:border-[#C9A84C]/50 focus:shadow-[0_0_10px_rgba(201,168,76,0.1)] transition-all" />
          </div>

          <div>
            <label className="block text-sm font-medium mb-2">Phone Number <span className="text-[#C9A84C]">*</span></label>
            <input type="tel" name="phone" value={formData.phone} onChange={handlePhoneChange} placeholder="(555) 000-0000" maxLength={14} className="w-full px-4 py-3 rounded-xl bg-[#141414] border border-[#2A2A2A] text-white placeholder-[#666] focus:outline-none focus:border-[#C9A84C]/50 focus:shadow-[0_0_10px_rgba(201,168,76,0.1)] transition-all" />
          </div>

          <div>
            <label className="block text-sm font-medium mb-2">Business / Brand Name <span className="text-[#C9A84C]">*</span></label>
            <input type="text" name="business_name" value={formData.business_name} onChange={handleChange} placeholder="Your company or brand" className="w-full px-4 py-3 rounded-xl bg-[#141414] border border-[#2A2A2A] text-white placeholder-[#666] focus:outline-none focus:border-[#C9A84C]/50 focus:shadow-[0_0_10px_rgba(201,168,76,0.1)] transition-all" />
          </div>

          <div>
            <label className="block text-sm font-medium mb-4">Package Interest <span className="text-[#C9A84C]">*</span></label>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">

              <button type="button" onClick={() => selectPackage('standard')} className={`text-left rounded-2xl border p-5 transition-all duration-300 ${formData.package_interest === 'standard' ? 'border-[#C9A84C] shadow-[0_0_20px_rgba(201,168,76,0.2)]' : 'border-[rgba(201,168,76,0.15)] hover:border-[rgba(201,168,76,0.3)]'} bg-[#0D0D0F]`}>
                ...
                <ul className="space-y-2">
                  {STANDARD_FEATURES.map((f, i) => (
                    <li key={i} className="flex items-start gap-2 text-xs text-[#CCCCCC]">
                      <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#C9A84C" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="shrink-0 mt-0.5"><polyline points="20 6 9 17 4 12"/></svg>
                      {f}
                    </li>
                  ))}
                </ul>
              ...
              <div className="relative">
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 z-10">
                  <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider bg-[rgba(201,168,76,0.15)] text-[#C9A84C] border border-[#C9A84C]/30">
                ...
                <button type="button" onClick={() => selectPackage('premium')} className={`w-full text-left rounded-2xl border p-5 transition-all duration-300 ${formData.package_interest === 'premium' ? 'border-[#C9A84C] shadow-[0_0_20px_rgba(201,168,76,0.2)]' : 'border-[rgba(201,168,76,0.15)] hover:border-[rgba(201,168,76,0.3)]'} bg-[#0D0D0F]`}>
          ...
          <div className="text-center pt-4">
            <p className="inline-flex items-center gap-2 text-[#C9A84C] text-xs sm:text-sm font-semibold">
